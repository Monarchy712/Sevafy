    ...
    <div className={styles.dashboard}>
      ...
      <header className={styles.header}>
        <div className={styles.headerContent}>
          <div>
            <div className={styles.headerTag}>NGO Management Portal</div>
      ...
      <div className={styles.tabContent}>
        ...
        {activeTab === 'donations' && (
          <div className={styles.donationTable}>
            ...
            {donations.length === 0 && (
              <div className={styles.emptyState}>No donations yet. Share your NGO profile to start receiving funds.</div>
            )}
            {donations.map((d, i) => (
              <div key={d.donation_id} className={styles.tableRow} style={{ animationDelay: `${i * 0.05}s` }}>
                <span className={styles.donorCell}>
                  <span className={styles.donorAvatar}>{d.donor_name[0]}</span>
                  {d.donor_name}
                </span>
                <span className={styles.amountCell}>{fmt(d.amount)}</span>
                <span className={styles.dateCell}>{fmtDate(d.donated_at)}</span>
                <span>
                  {d.confirmed
                ...
                <span className={styles.txHash}>
                  {d.tx_hash
                    ? <code title={d.tx_hash}>{d.tx_hash.slice(0, 10)}…</code>
        ...
        {activeTab === 'scholarships' && !selectedApp && (
          <div className={styles.scholarshipGrid}>
            {scholarships.length === 0 && (
              <div className={styles.emptyState}>No scholarship applications yet.</div>
            )}
            {scholarships.map((s, i) => (
              ...
              >
                <div className={styles.scardHeader}>
                  <div className={styles.scardAvatar}>{s.student_name[0]}</div>
                  <div>
                    <div className={styles.scardName}>{s.student_name}</div>
                    <div className={styles.scardScheme}>{s.scheme_title}</div>
                  </div>
                  <StatusBadge status={s.status} />
                </div>

                <div className={styles.scardMeta}>
                  <span>💸 {fmt(s.amount_per_student)}</span>
                  <span>📅 {fmtDate(s.applied_at)}</span>
                  {s.verified_by_genai !== null && (
                    <span>{s.verified_by_genai ? '✅ AI Verified' : '❌ AI Rejected'}</span>
                  )}
                </div>

                {/* Mini progress bar */}
                <div className={styles.miniProgress}>
                  <div className={styles.miniProgressLabel}>
                    Progress: Stage {s.phases_completed}/12
                  </div>
                  <div className={styles.miniProgressTrack}>
                    <div
                      className={styles.miniProgressFill}
                      style={{ width: `${(s.phases_completed / 12) * 100}%`, background: tier.color }}
                    />
                  </div>
                </div>
